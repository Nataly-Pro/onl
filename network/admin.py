...
class NetworkObjectAdmin(admin.ModelAdmin):
    list_display = [f.name for f in NetworkObject._meta.fields] + ['supplier_link']
    ...
    def supplier_link(self, obj):
        if obj.supplier:
            return mark_safe('<a href="{}">{}</a>'.format(reverse('admin:network_networkobject_change',
                                                                  args=(obj.supplier.pk,)), obj.supplier))
        else:
            return None
